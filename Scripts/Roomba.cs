...
public class Roomba : MonoBehaviour
{
    ...
    [SerializeField] private GameObject roombaText;
    ...
    private const int CorrectChangeRouteNum = 1; //正しいルンバのルートのための値
    private const int IncorrectChangeRouteNum = 2; //間違ったルンバのルートのための値
    [Header("正しいルンバのルートID")]
    [SerializeField] private int correctRouteMinID = 0;
    [SerializeField] private int correctRouteMaxID = 2;
    [Header("間違ったルンバのルートID")]
    [SerializeField] private int incorrectRouteMinID = 3;
    [SerializeField] private int incorrectRouteMaxID = 5;
    ...
    public void BackToOriginalRoute()
    {
        if (isSetup == true)
        {
            routeSetting = 0;
    ...
    ///ルート変更
    /// </summary>
    public void ChangeRouteSetting()
    {
        if(isSetup == true)
        {
            if (IsCorrectRoombaRoute())
            {
                routeSetting = CorrectChangeRouteNum;
            }
            else
            {
                routeSetting = IncorrectChangeRouteNum;
            }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            roombaText.SetActive(true);
        }
    }
    private void OnTriggerStay(Collider other)
    {
        if (other.gameObject.CompareTag("Player") && Input.GetKey(KeyCode.E)&&!isSetup)
        {
